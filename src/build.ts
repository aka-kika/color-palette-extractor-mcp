...
// Escape a value for safe interpolation into the generated HTML guide. The
// guide is rendered by headless Chrome, so unescaped source filenames / labels
// would otherwise be an HTML/JS injection vector.
...
function deriveOtherMode(swatches: any[], sourceMode: "dark" | "light"): any[] {
  ...
  return swatches.map((s) => {
    ...
    // targetS is an absolute target saturation (0–1 → 0–100%). Preserve true
    // grays (s≈0) so we don't invent a hue for a neutral swatch; otherwise pull
    // the swatch to the role's target saturation (e.g. ~85% for accents).
    const newHex = rgbToHex(hslToRgb({
      h: hsl.h, s: hsl.s <= 2 ? hsl.s : Math.min(100, targetS * 100), l: targetL,
    }));
    return {
...
function htmlGuide(opts: any): string {
  ...
  const renderCard = (s: any) => {
    ...
    return `
    <div class="swatch">
      <div class="chip" style="background:${esc(s.hex)}; color:${fg}">
        <span class="role">${esc(s.role)}</span>
        <span class="hex">${esc(s.hex.toUpperCase())}</span>
  ...
  const renderThemeBlock = (themeLabel: string, swatches: any[], id: string) => {
    ...
    const rows = swatches.map((s: any) => {
      ...
      return `
        <tr>
          <td><div class="dot" style="background:${esc(s.hex)}"></div></td>
          <td><code>${esc(s.hex.toUpperCase())}</code></td>
...
<header>
  ...
  <div style="display:flex; gap:24px; align-items:center;">
    <div class="meta">source: <code>${esc(sourceMode)}</code> · generated by color-palette-mcp</div>
...
<main>
  <section>
    <h2>Source</h2>
    <div class="src-info">
      <div><span>Source</span><strong><code>${esc(src)}</code></strong></div>
...
export async function buildDeliverableFolder(
  ...
  // Screenshot the HTML guide. Use execFileSync with an argument array (no
  // shell) so the output directory can never be interpreted as a command —
  // OUT contains the caller-supplied output_dir.
...
- \`index.html\` — interactive design system guide (with dark/light toggle)${screenshotOk ? "\n- `index.png` — screenshot of the design system guide" : ""}
